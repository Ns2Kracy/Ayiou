...
- `SessionKey`（bot/platform/plugin/user/channel）
...
```rust
use ayiou::core::session::{MemorySessionStore, SessionKey, SessionStore};
use ayiou::core::storage::{MemoryStore, StoreSerdeExt};
use std::sync::Arc;
