...
async fn sea_orm_store_roundtrip_and_delete() {
    ...
    let url = format!("sqlite://{}?mode=rwc", dir.path().join("kv.sqlite").display());
    let store = SeaOrmStore::connect(&url).await.unwrap();

    store.set_json("plugin:test", &vec![1_u64, 2, 3]).await.unwrap();
...
async fn sea_orm_store_lists_keys_by_prefix() {
    ...
    assert_eq!(keys, vec!["plugin:a:1".to_string(), "plugin:a:2".to_string()]);
}
